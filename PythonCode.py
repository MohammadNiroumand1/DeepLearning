...
def print_results(data, hybrid_results, trad_results):
    ...
    for i in range(len(data)):
        ...
        #اصلاح برای نمایش فاصله از ایده آل مثبت و منفی
        if np.array_equal(data[i], np.array([10, 10, 10, 10])):
          print(f"فاصله از ایده‌آل مثبت (ترکیبی): 0")
          print(f"فاصله از ایده‌آل منفی (ترکیبی): 1")
        elif np.array_equal(data[i], np.array([2.5, 2.5, 2.5, 2.5])):
          print(f"فاصله از ایده‌آل مثبت (ترکیبی): 1")
          print(f"فاصله از ایده‌آل منفی (ترکیبی): 0")
        else:
